...
public class G1마법사상어와복제_23290 {
    ...
    private static void findSharkDir(int order) {
        ...
        for (int d1 = 0; d1 < 4; d1++) {
            int sr1 = sr+sdx[d1];
            int sc1 = sc+sdy[d1];        
            if(!outLineCheck(sr1, sc1)) continue; // 벽 밖이면 false return;
            for (int d2 = 0; d2 < 4; d2++) {
                int sr2 = sr1+sdx[d2];
                int sc2 = sc1+sdy[d2];
                if(!outLineCheck(sr2, sc2)) continue; // 벽 밖이면 false return;
                for (int d3 = 0; d3 < 4; d3++) {
                    int sr3 = sr2+sdx[d3];
                    int sc3 = sc2+sdy[d3];
                    if(!outLineCheck(sr3, sc3)) continue; // 벽 밖이면 false return;
